...
# Attempt to get MongoDB URI from the existing .env.live (if present) to avoid
# re-computing the PrivateLink URI on agent-only deploys.
MONGODB_URI=""
if [[ -f "$REPO_ROOT/.env.live" ]]; then
  while IFS='=' read -r key value; do
    if [[ "$key" == "MONGODB_URI" ]]; then
      MONGODB_URI="$value"
      break
    fi
  done < "$REPO_ROOT/.env.live"
fi
if [[ -z "$MONGODB_URI" ]]; then
  err "Could not read MONGODB_URI from .env.live. Refusing to update runtime env vars because update-agent-runtime replaces the whole environment map. Run deploy.sh once, or restore .env.live, then rerun deploy-agents.sh."
fi
