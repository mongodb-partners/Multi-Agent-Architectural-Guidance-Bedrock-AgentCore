#!/usr/bin/env bash
# reap-orphan-security-groups-privatelink.sh — Delete security groups left behind
# during a PrivateLink project destroy because service-managed AgentCore ENIs
...
#
# Usage:
#   ./deploy/destroy/reap-orphan-security-groups-privatelink.sh [options]
#
# Options:
...
log()  { echo "  [reap-sg:privatelink] $*"; }
ok()   { echo "  [reap-sg:privatelink] ✓ $*"; }
err()  { echo "  [reap-sg:privatelink] ✗ $*" >&2; exit 1; }
warn() { echo "  [reap-sg:privatelink] ⚠ $*"; }
sep()  { echo "------------------------------------------------"; }
