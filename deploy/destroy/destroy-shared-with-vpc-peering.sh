#!/usr/bin/env bash
# destroy-shared-with-vpc-peering.sh - Tear down shared resources in VPC peering mode.
#
# Usage:
...
#     (reap-orphan-security-groups-vpc-peering.sh) after AWS releases them, then re-run this script.
#
# This is for shared/singleton teardown. Run destroy-project-with-vpc-peering.sh
...
warn_if_agentcore_enis_present() {
  ...
  warn "    ./deploy/destroy/reap-orphan-security-groups-vpc-peering.sh --watch"
...
NETWORK_MODE="$CHILD_NETWORK_MODE" ATLAS_PEERING_CIDR="$ATLAS_PEERING_CIDR" bash "$DESTROY_SCRIPT" --mode shared "${CHILD_ARGS[@]}"

NETWORK_ARGS=("${CHILD_ARGS[@]}")
[[ "$WITH_BOOTSTRAP" == "true" ]] && NETWORK_ARGS+=("--with-bootstrap")
warn_if_agentcore_enis_present
NETWORK_MODE="$CHILD_NETWORK_MODE" ATLAS_PEERING_CIDR="$ATLAS_PEERING_CIDR" bash "$DESTROY_SCRIPT" --mode network "${NETWORK_ARGS[@]}"
...
if [[ -s "$ORPHAN_SG_MANIFEST" ]]; then
  ...
  warn "Run ./deploy/destroy/reap-orphan-security-groups-vpc-peering.sh --watch after ENIs clear, then re-run this script if network destroy did not finish."
fi
