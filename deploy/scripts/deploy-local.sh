...
# Wrap `terraform apply` with retry-on-transient-error. The MongoDB Atlas
# control plane (cloud.mongodb.com) occasionally returns i/o timeouts or
# connection-resets that vanish on the next call, and a local DNS resolver
# blip can surface mid-apply too. We retry the apply up to `max_attempts - 1`
# times, re-planning between attempts so the saved plan stays consistent with
# the post-partial-apply state. Any non-transient error is treated as a hard
# failure and stops the script immediately.
apply_with_retry() {
  local plan_file="$1"
  ...
  while (( attempt <= max_attempts )); do
    ...
    if deploy_log_has_transient_error "$log_file" \
      ...
      warn "Transient Atlas/network error detected on attempt ${attempt} — will re-plan and retry"
      attempt=$((attempt + 1))
      continue
    fi
    rm -f "$log_file"
...
# validated value to envs/local as var.voyage_output_dim.
...
# ── Operator/laptop IP for the Atlas IP access list ─────────────────────────
# Local mode runs the mongodb-atlas module in its default 'privatelink' shape,
# so this /32 is the ONLY Atlas IP access list entry (never 0.0.0.0/0). Without
# it your laptop cannot reach its own local cluster.
# shellcheck source=deploy/scripts/_operator-ip.sh
source "$SCRIPT_DIR/_operator-ip.sh"
resolve_operator_ip_cidr "local" || err "Could not determine operator IP for the Atlas IP access list. Atlas must be reachable from this machine without 0.0.0.0/0. Set OPERATOR_IP_CIDR=A.B.C.D/32 in .env (or TF_VAR_my_ip) and re-run."
...
# Operator/laptop public IP — the ONLY Atlas IP access list entry in local mode
# (replaces the former 0.0.0.0/0). Auto-detected by deploy-local.sh; override
# via OPERATOR_IP_CIDR / TF_VAR_my_ip in .env.
...
if [[ "$SKIP_SEED" == "true" ]]; then
  ...
  # Embedding dim defaults to 1024 and is resolved through the Voyage SSOT
  # (getVoyageEmbeddingDims / voyage_embedding_dims) when Voyage is active.
