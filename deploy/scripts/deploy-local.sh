...
# KB ID is written by bedrock-kb module's provisioner script; re-read post-apply
BEDROCK_KB_ID=""
...
# Re-read KB ID now that apply has run
if [[ -f "$KB_STATE_FILE" ]]; then
  BEDROCK_KB_ID=$(python3 -c "import json; print(json.load(open('$KB_STATE_FILE')).get('knowledge_base_id',''))" 2>/dev/null || echo "")
fi
[[ -n "$BEDROCK_KB_ID" ]] && ok "Bedrock KB: $BEDROCK_KB_ID" || warn "KB ID not found — check $KB_STATE_FILE"

# ══════════════════════════════════════════════════════════════════════════════
# PHASE 6 — Write .env.live
# ══════════════════════════════════════════════════════════════════════════════
sep
log "Phase 6 — Writing .env.live (local mode: Titan embeddings, direct tools)..."
...
# Tools:     Direct in-process (no AgentCore Gateway — that's EC2-only)
...
# Tool hosting: local mode always uses in-process tools (no AgentCore Gateway).
# For gateway-routed tools, deploy to EC2 (deploy.sh).
export TOOL_HOSTING_MODE="direct"
...
# Cognito — intentionally not wired for local dev so the UI skips the login screen.
