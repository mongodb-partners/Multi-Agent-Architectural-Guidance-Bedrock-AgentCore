...
// capture, and the empty-result clause-walker.
//
// Mirrors `api/src/lib/mongo-diagnostic.ts`. We keep this as plain JS so the
// Lambda zip needs no build step. The two files emit identical event payload
// shapes so the Trace Viewer renders Lambda-emitted and API-emitted events the
// same way.
