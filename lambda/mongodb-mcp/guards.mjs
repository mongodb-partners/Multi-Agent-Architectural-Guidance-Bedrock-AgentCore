// Shared MongoDB MCP guards — single source of truth for the validation rules
// applied to `mongodb_query`-style calls.
//
// Imported by BOTH:
//   - lambda/mongodb-mcp/index.mjs            (this directory)
//   - api/src/adapters/mongo-data.ts          (relative path:
//                                              ../../../lambda/mongodb-mcp/guards.mjs)
//
// Keep this file pure JS (no TypeScript syntax) so Lambda can `import` it as-is
// with no build step. TypeScript consumers pick up types from the companion
// `guards.d.mts` file alongside this one.
//
// Anything that depends on the trace collector, fixtures, MongoDB driver, or
// any other side-effectful concern stays out of this module — both sides need
// to be free to layer their own wrappers around the guards.
