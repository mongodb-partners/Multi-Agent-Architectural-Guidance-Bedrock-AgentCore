/**
 * Integration probe: MongoDB MCP runtime → API client trace round-trip contract.
 *
 * The MongoDB MCP runtime (mcp-runtimes/mongodb-mcp/src/server.ts, sharing
 * `src/vendor/handlers.mjs` + `src/vendor/tracing.mjs`) packs per-invocation
 ...
 *   })
 *
 * On the API side, `extractAndReplayMcpTraces` (in
 ...
 *      mongo.* cards whether the operation ran in-process or through the
 *      AgentCore Gateway → MCP runtime path.
 ...
 * (AgentCore Gateway) path — exactly the path client demos use. This test
 * pins the contract.
 */

import { describe, expect, test } from "bun:test";
import { extractAndReplayMcpTraces } from "../../src/adapters/mongodb-mcp-client.ts";
...
/**
 * Build an envelope identical in shape to what the MongoDB MCP runtime returns
 * (see mcp-runtimes/mongodb-mcp/src/server.ts + vendor/handlers.mjs).
 */
...
describe("MongoDB MCP runtime trace round-trip contract", () => {
  test("replays mongo.* events into the active TraceCollector and strips meta from content", () => {
    ...
    const envelope = makeEnvelope(
      ...
      [
        ...
        {
          type: "mongo.query",
          payload: { mode: "mcp-runtime", collection: "orders", op: "find", limit: 50 },
        },
        {
          type: "mongo.schema",
    ...
    const dropped = withTrace(collector, () => extractAndReplayMcpTraces(envelope, collector));
    ...
    expect((queryEv.payload as { mode: string }).mode).toBe("mcp-runtime");
    ...
    // Collector stamps its own id / ts / agentId on every event — runtime-side
  ...
  test("accumulates tracesDropped across all envelope blocks", () => {
    ...
    const dropped = withTrace(collector, () => extractAndReplayMcpTraces(result, collector));
  ...
  test("no-ops with zero events on results lacking a content[] array", () => {
    ...
    expect(withTrace(collector, () => extractAndReplayMcpTraces(null, collector))).toBe(0);
    expect(withTrace(collector, () => extractAndReplayMcpTraces(undefined, collector))).toBe(0);
    expect(withTrace(collector, () => extractAndReplayMcpTraces({}, collector))).toBe(0);
    expect(withTrace(collector, () => extractAndReplayMcpTraces({ content: "not-an-array" }, collector))).toBe(0);
    expect(withTrace(collector, () => extractAndReplayMcpTraces({ content: [] }, collector))).toBe(0);
  ...
  test("when no TraceCollector is active, still strips meta from content without throwing", () => {
    ...
    const dropped = extractAndReplayMcpTraces(envelope, undefined);
