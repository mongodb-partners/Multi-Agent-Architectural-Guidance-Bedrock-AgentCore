...
|---|---|---|
| `.env` | AWS credentials, Atlas creds, project name (gitignored) | Sourced manually before `bun run dev` or `deploy-full-with-privatelink.sh` |
| `.env.live` | Generated by `deploy-project.sh` Phase 9. Runtime config (ARNs, URIs, mode flags) | Sourced by EC2 systemd unit; sourced manually for local dev |
...
In production (EC2), this is set by `deploy-project.sh` to the orchestrator runtime ARN.
...
./deploy/deploy-full-with-privatelink.sh --auto-approve  # provisions the endpoint, wires env vars
...
When `EMBEDDINGS_PROVIDER=voyage`, `deploy-project.sh` validates the selected Voyage model/request format and sets `VOYAGE_SAGEMAKER_ENDPOINT`, `VOYAGE_OUTPUT_DIM=1024`, and `VOYAGE_REQUEST_FORMAT` on all surfaces that need them:
...
Set `EMBEDDINGS_PROVIDER=titan` in `.env` and re-run `deploy-full-with-privatelink.sh`. The next apply skips the SageMaker module (`count = 0`), the runtimes get `VOYAGE_SAGEMAKER_ENDPOINT=""`, and the adapters use `bedrockGenerateEmbedding(EMBEDDING_MODEL_ID)`. Then re-seed:
...
# dev Cognito pool (deploy-project.sh writes them into .env.live, or copy from your dev
# environment).
