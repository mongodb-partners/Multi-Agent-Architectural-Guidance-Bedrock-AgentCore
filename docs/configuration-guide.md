...
| `AGENTCORE_MEMORY_STORE_ID` | `<project>_memory_<env>-<suffix>` | LTM AgentCore fallback + short-term backend when `SHORT_TERM_MEMORY_BACKEND=agentcore` |
...
# environment).
export AUTH_JWKS_URI="$(jq -r '.cognito_jwks_uri' deploy-manifest.json)"
export AUTH_ISSUER="$(jq -r '.cognito_issuer' deploy-manifest.json)"
cd api && bun run dev
