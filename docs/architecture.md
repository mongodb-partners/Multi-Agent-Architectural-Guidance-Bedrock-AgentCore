...
- **Multi-specialist orchestration.** When the classifier (`classifyAgents`) returns 2+ specialists for a single message, the API invokes each specialist in classifier-ranked order, streams each draft with `phase: "specialist"` SSE token metadata, then runs an in-process **synthesizer agent** (transient Strands `Agent`, `id: "synthesizer"`, no AgentCore runtime, no `.agent.md`) to compose one cohesive customer answer streamed as `phase: "synthesis"` tokens. Only the synthesis text is persisted (`agentId: "orchestrator"`); specialist drafts live in trace + live UI only. Single-domain prompts take a fast path: one specialist call, no synthesis, no token-`phase` field, specialist tokens persist directly. Knobs: `CLASSIFIER_MULTI_MIN_SCORE` (default `3.0`), `CLASSIFIER_MULTI_RELATIVE_MARGIN` (default `1.5`), `CLASSIFIER_MULTI_MAX_AGENTS` (default `2`). The `synthesizer` Bedrock call is tagged `agentId: "synthesizer"` for cost attribution via `MetadataAwareBedrockModel`. Trace events: `orchestrator.multi_route_decision`, `orchestrator.specialist_draft` (one per specialist), `orchestrator.synthesis`. See [`api/src/lib/multi-specialist-orchestrator.ts`](../api/src/lib/multi-specialist-orchestrator.ts) and [`api/src/lib/specialist-answer-synthesizer.ts`](../api/src/lib/specialist-answer-synthesizer.ts).
